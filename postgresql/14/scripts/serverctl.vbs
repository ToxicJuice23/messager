...
Select Case strAction
    Case "start"
         iRet = DoCmd("""" & strWinDir & """\System32\net start postgresql-x64-14")
    Case "stop"
         iRet = DoCmd("""" & strWinDir & """\System32\net stop postgresql-x64-14")
    Case "reload"
         iRet = DoCmd("""C:\Program Files\PostgreSQL\14\bin\pg_ctl.exe"" -D ""C:\Program Files\PostgreSQL\14\data"" reload")
    Case "restart"
	     iRet = DoCmd("""" & strWinDir & """\System32\net stop postgresql-x64-14")
		 If iRet = 0 Then
		     iRet = DoCmd("""" & strWinDir & """\System32\net start postgresql-x64-14")
